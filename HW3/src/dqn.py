...
class QLearner(object):
  ...
  def __init__(
    ...
    # Get the optimal actions for current network
    self.greedy_action = tf.argmax(q_t, axis=1)
    
    # Initialize target network
    q_tp1 = q_func(obs_tp1_float, self.num_actions, scope="target_q_func", reuse=False)
    
    # Get indexes of optimal actions from current network for selecting them in the target network (Double Q)
    indexes  = [[i, self.greedy_action[i]] for i in range(0, self.batch_size)]
    
    # Clone of Double Q flag for simplicity when modifying the code in this section 
    double_q = False
    
    # If Double Q, we take the actions from the current Q and select them in the target Q using indexes array
    if double_q: 
        # Compute modified target
        target = self.rew_t_ph + (1.0 - self.done_mask_ph) * gamma *  tf.gather_nd(q_tp1, indices=indexes) 
    ...
    # Q values from current network from the current actions
    q_t_act = tf.reduce_sum(q_t * tf.one_hot(self.act_t_ph, self.num_actions), axis=1)
    
    # Error functions: huber_loss / MSE. Uncomment/comment as needed
    self.total_error = huber_loss(target - q_t_act, delta=1.0) 
    #self.total_error = tf.losses.mean_squared_error(target, q_t_act)  
  ...
  def step_env(self):
    ...
    #####
    # YOUR CODE HERE
    # Get idx for replay buffer 
    self.replay_buffer_idx = self.replay_buffer.store_frame(self.last_obs)
    
    # Random actions at the beginning
    if not self.model_initialized or random.random() < self.exploration.value(self.t):
        action = random.randint(0, self.num_actions - 1)
    
    # Else, use our agent knowledge
    else:
        obs = self.replay_buffer.encode_recent_observation()
        action = self.session.run(self.greedy_action, {self.obs_t_ph: [obs]})
        if len(action) == 1:       # For lunar landing: actions are scalar
            action = action[0]
        
    # Debug
    # print("Action:", action)
    
    # Apply action and collect new state
    next_obs, reward, done, _ = self.env.step(action)
    
    # If done, reset and get the last observation
    if done:
        self.last_obs = self.env.reset()
    
    # Store and get new observation
  ...
  def update_model(self):
    ...
    if (self.t > self.learning_starts and \
      ...
      obs_batch, act_batch, rew_batch, obs_tp1_batch, done_batch = self.replay_buffer.sample(self.batch_size)
      
      # If model not initialized, initialize it
      if not self.model_initialized:
        initialize_interdependent_variables(self.session, tf.global_variables(), {self.obs_t_ph: obs_batch, self.obs_tp1_ph: obs_tp1_batch,})
  ...
  def log_progress(self):
    ...
    if self.t % self.log_every_n_steps == 0 and self.model_initialized:
      ...
      # Append value for .pkl
      self.log_episodes.append(len(episode_rewards))
      print("exploration %f" % self.exploration.value(self.t))
      
      # Append value for .pkl
      self.log_exploration.append(self.exploration.value(self.t))
      print("learning_rate %f" % self.optimizer_spec.lr_schedule.value(self.t))
      
      # Append value for .pkl
      self.log_lr.append(self.optimizer_spec.lr_schedule.value(self.t))
      ...
      sys.stdout.flush()
    if self.t % 100000 == 0 and self.model_initialized:
      training_log = ({'log_t': self.log_t, 'log_mean_reward': self.log_mean_reward,  'log_best_mean': self.log_best_mean_reward, 'log_episodes': self.log_episodes,
                              'log_exploration': self.log_exploration, 'log_learning_rate': self.log_lr})
      output_file_name = str(self.rew_file)+'_' + str(self.t) + '_data.pkl'
