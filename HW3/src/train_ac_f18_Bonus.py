...
class Agent(object):
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        if self.discrete:
            ...
            # YOUR_HW2 CODE_HERE
            sy_mean = build_mlp(sy_ob_no, self.ac_dim, 'policy_mean', n_layers=self.n_layers, size=self.size)
    ...
    def build_computation_graph(self):
        ...
        # New modification: Actor loss instead of loss and we update it
        ...
        # define the critic: NEW CODE!!!!: We define a neural network for the critic 
        # Critic NN parameters 
        self.critic_n_layers = 2 * self.n_layers
        self.critic_lr = 5e-2  #1e-3
        self.critic_size = 2 * self.size
        
        self.critic_prediction = tf.squeeze(build_mlp(
                                self.sy_ob_no,
                                1,
                                "nn_critic",
                                n_layers=self.critic_n_layers,
                                size=self.critic_size))
        self.sy_target_n = tf.placeholder(shape=[None], name="critic_target", dtype=tf.float32)
        #self.sy_target_n = tf.stop_gradient(self.sy_target_n)
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        # Get V(s') from the critic_prediction using the next state observation
        V_t1_vals = self.sess.run(self.critic_prediction, feed_dict = {self.sy_ob_no: next_ob_no})
        #print("V(s'):", V_t1_vals)
        #print("V(s') shape:", V_t1_vals.shape)
        #print("Re_n:", re_n)
        #print("Re_n shape:", re_n.shape)
            
        # Calculate q 
        q_n = re_n + (1.0 - terminal_n) *  self.gamma *  V_t1_vals
        #print("Q:", q_n)
        #print("Q shape:", q_n.shape)
        #for path in re_n:
           # q = 0
            #q_path = []
        ...
        # Get V(s) values from the prediction operation using current observation    
        V_t_vals = self.sess.run(self.critic_prediction, feed_dict = {self.sy_ob_no: ob_no})    
    ...
    def update_critic(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        for _ in range(self.num_target_updates):
            #print("Updating Critic number: ", _)
            # Update the target values        
            # Calculate next state values using critic network and next observation (s')
            V_t1_vals = self.sess.run(self.critic_prediction, feed_dict = {self.sy_ob_no: next_ob_no})
            #print("V(s'):", V_t1_vals, "\nShape:", V_t1_vals.shape)
            #print("Re_n:", re_n)
            #print("Re_n shape:", re_n.shape)
            target_n = re_n + (1.0 - terminal_n) *  self.gamma *  V_t1_vals
            #print("Target:", target_n, "shape:", target_n.shape)
            #target_n = []
            #for path in re_n:
             #   target_n = 0
              #  target_path = []
                ...
                #for rewards in reversed(path):
				  ...
				  #target_path.append(target_n)
                
                # Reverse the values for consistency with time period
                #target_path.reverse()
                #target_path = [target_path[0] * len(target_path)]
                #target_n.extend(target_path)
        
            
            # Run the some gradient steps
            for i in range(self.num_grad_steps_per_target_update): 

                # Data to feed
                feed_dict = {self.sy_target_n: target_n, self.sy_ob_no: ob_no}

                # Call the session, extract info, summary to save and loss function 
                _, loss = self.sess.run([self.critic_update_op, self.critic_loss], feed_dict=feed_dict)

                # Display loss
                #print("Critic's loss: %f" % loss, "Step:", i)
            
#             # For each gradient step, update with current target
#             for __ in range(self.num_grad_steps_per_target_update):

#                 self.sess.run(self.critic_update_op, 
#                               feed_dict={self.sy_target_n: target, self.sy_ob_no: ob_no})
...
def train_AC(
    ...
    for itr in range(n_iter):
        ...
        # Compute advantage via agent.estimate_advantage
        adv_n = agent.estimate_advantage(ob_no, next_ob_no, re_n, terminal_n)
        
        # Update the actor via agent.update_actor
