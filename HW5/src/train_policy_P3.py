...
from point_mass_P3 import PointEnv
...
def build_rnn(x, h, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None, regularizer=None):
    ...
    # MLP Layers (CP: checking dimensions and consistency...)
    #print("X shape[1]:", x.get_shape()[1])
    for i in range(x.get_shape()[1]):
        input_x = x[:, i, :]
        #print("Input x:", input_x)
        #input_x = tf.reshape(input_x, (-1, input_x.get_shape()[1]*input_x.get_shape()[2]))
        input_x = build_mlp(input_x, output_size, scope, n_layers + 1, size, activation, output_activation, regularizer)
        
        # New logic by cell
        output, h = cell(inputs=input_x, state=h, scope=scope)
...
def build_policy(x, h, output_size, scope, n_layers, size, gru_size, recurrent=True, activation=tf.tanh, output_activation=None):
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        if recurrent:
            #print("\nX recurrent pre build RNN:", x)
            #print("h recurrent pre build RNN:", h)
            x, h = build_rnn(x, h, gru_size, scope, n_layers, 
                             size, activation=activation, 
                             output_activation=output_activation)
            #print("\nX recurrent post build RNN:", x)
            #print("h recurrent post build RNN:", h)
            
        else:
            x = tf.reshape(x, (-1, x.get_shape()[1]*x.get_shape()[2]))
            #print("\nX No recurrent pre build mlp:", x)
            x = build_mlp(x, gru_size, scope, n_layers + 1, size, activation=activation, output_activation=activation)
            #print("X No recurrent post build mlp:", x)
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        if recurrent:
            ...
            x = build_mlp(x, gru_size, scope, n_layers + 1, size, 
                          ...
                          regularizer=regularizer)
        x = tf.layers.dense(x, output_size, activation=output_activation, 
                            name='decoder', kernel_regularizer=regularizer, 
                            bias_regularizer=regularizer)
    return x
...
class Agent(object):
    ...
    def build_computation_graph(self):
        ...
        self.critic_prediction = tf.squeeze(build_critic(self.sy_ob_no, self.sy_hidden, 1, 
                                                         'critic_network', n_layers=self.n_layers, 
                                                         size=self.size, gru_size=self.gru_size, 
                                                         recurrent=self.recurrent, 
    ...
    def sample_trajectories(self, itr, env, min_timesteps, is_evaluation=False, sqs=-1):
        # Collect paths until we have enough timesteps
        timesteps_this_batch = 0
        stats = []
        while True:
            animate_this_episode=(len(stats)==0 and (itr % 10 == 0) and self.animate)
            steps, s = self.sample_trajectory(env, animate_this_episode, is_evaluation=is_evaluation, sqs=sqs)
            stats += s
    ...
    def sample_trajectory(self, env, animate_this_episode, is_evaluation, sqs):
        """
        ...
        debug = False
        if debug:
            print("Obs dimension:", self.ob_dim, 
                  "\nAc dimension:", self.ac_dim,
                  "\nTask dimension:", self.task_dim,
                  "\nReward dimension:", self.reward_dim,
                  "\nTerminal dimension:", self.terminal_dim)
        #print("sqs inside sample trajectory:", sqs)
        env.reset_task(is_evaluation=is_evaluation, square_size=sqs)
        stats = []
        #====================================================================================#
        #                           ----------PROBLEM 2----------
        #====================================================================================#
        ...
        in_ = np.zeros((self.history, meta_obs.shape[1]))
        
        if debug:
            print("Meta obs (Initial):\n", meta_obs)
        rewards = []
        ...
        while True:
            ...
            if ep_steps == 0:
                ob = env.reset()
                if debug:
                    print("\nInitial observation after reset:", ob)
                ...
                # Contatenate ob, a, r, d and save them to the meta_obs array
                meta_obs[steps] = np.concatenate((ob, a, r, d), axis=None)
                if debug:
                    print("\nInitial meta_obs:\n", meta_obs)
                steps += 1
            ...
            # YOUR CODE HERE
            # That variable should be the windowed observation, an array of history length x meta_obs_dim.
            if steps < self.history:
                if debug:
                    print("Steps < History")
                    print("Steps:", steps)
                in_[self.history - steps:] = meta_obs[:steps]
                #in_[:steps] = meta_obs[:steps][::-1][:steps].copy()
            else:
                if debug:
                    print("Steps >= History")
                    print("Steps:", steps)
                in_ = meta_obs[steps - self.history:steps].copy()   #[:-self.history-1:-1] #[steps - 1 : steps - 1 + self.history : 1]  #
            
            # Reversed (current step at the end of the array) not needed 12 Nov
            #in_ = in_[::-1]
            ...
            if debug:
                print("Ob, rew, done obtained from env.step(action):\n\t", ob, "\n\t", rew, "\n\t", done)
            ...
            # YOUR CODE HERE
            meta_obs[steps] = np.concatenate((ob, ac, [rew], [done]), axis=None)
            if debug:
                print("MetaObs new:", meta_obs[steps])
                print("\nMetaObs:", meta_obs)
            rewards.append(rew)
            steps += 1
            ...
            if steps < self.history:
                ...
                if debug:
                    print("Steps >= History")
                    print("Steps:", steps)
                in_ = meta_obs[steps - self.history:steps].copy()
            if debug:
                print("New in_:", in_)
    ...
    def update_policy(self, ob_no, hidden, ac_na, fixed_log_probs, advantages):
        ...
        arguments:
            ...
            hidden: (minibsize, self.gru_size)
        '''
        policy_surr_loss, _ = self.sess.run([self.policy_surr_loss, self.policy_update_op],
                                            feed_dict={self.sy_ob_no: ob_no, 
                                                       self.sy_hidden: hidden, 
                                                       self.sy_ac_na: ac_na, 
                                                       self.sy_fixed_lp_n: fixed_log_probs, 
...
def train_PG(
    ...
    env = envs[env_name](num_tasks, sqs=sqs)
    ...
    for itr in range(n_iter):
        ...
        for _ in range(num_tasks):
            s, timesteps_this_batch = agent.sample_trajectories(itr, env, min_timesteps_per_batch, sqs=sqs)
        ...
        # Checking unpacked data:
        if DEBUG:
            print("\n ---- Checking unpack ----",
                  "\n\tob_no:", ob_no, ob_no.shape,
                  "\n\tac_na:", ac_na, ac_na.shape,
                  "\n\tre_n:", re_n, re_n.shape,
                  "\n\thidden:", hidden, hidden.shape,
                  "\n\tmask:", masks, masks.shape)
...
def main():
    ...
    parser.add_argument('--square_size', '-sqs', type=float, default=-1)
