...
def plot_data(data, value="AverageReturn", namePlot="Plot", label=""):
    ...
    g2 = sns.tsplot(data=data, time="Iteration", value="ValAverageReturn", unit="Unit", condition="Condition", color="r")
    g = sns.tsplot(data=data, time="Iteration", value=value, unit="Unit", condition="Condition")
    g.set(xlim=(0, data["Iteration"].max() + 1))
    g2.set(xlim=(0, data["Iteration"].max() + 1))
    #sns.tsplot.legend([g, g2], labels=["Training AVGReturns", "Test AVGReturns"])
    plt.legend(loc='best', labels=[label + "_Test", label + "_Train"]).draggable()
    #plt.legend([g, g2], ['Training', 'Testing'])
...
def main():
    ...
    if isinstance(args.value, list):
        ...
        values = [args.value]
    for value in values:
        plot_data(data, value=value, namePlot=args.plotName, label=args.label)
