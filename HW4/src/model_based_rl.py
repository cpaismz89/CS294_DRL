# CS 294-112 HW4
# Code modified by: Cristobal Pais
# Date: October 2018
...
class ModelBasedRL(object):
    ...
    def __init__(self,
        ...
        # nn_layers bug fix! (added to the ModelBasedPolicy object)
    ...
    def _train_policy(self, dataset):
        ...
        losses = []
        ### PROBLEM 1
        # a) Train the model-based policy for asked epochs
        for epoch in tqdm(range(self._training_epochs)):
            # For each batch inside the dataset
            for batch in dataset.random_iterator(self._training_batch_size):
                # Extract state, action, and next state from batch 
                states, actions, next_states, _, _ = batch
                
                # Call the train step from ModelBasedPolicy object
                loss = self._policy.train_step(states, actions, next_states)
                
                # Append new loss for tracking
                losses.append(loss)
    ...
    def run_q1(self):
        ...
        ### PROBLEM 1
        # (i) Train using the random dataset 
        ...
        for r_num, (states, actions, _, _, _) in enumerate(self._random_dataset.rollout_iterator()):
            print("Evaluation", r_num+1)
            pred_states = []

            ### PROBLEM 1
            # (ii) Use initial state and all actions in rollout to predict future states, 
			# keeping them in array pred_states 
            state = states[0]
            for _, action in tqdm(enumerate(actions)):
    ...
    def run_q3(self):
        ...
        for itr in range(self._num_onpolicy_iters + 1):
            ...
            ### PROBLEM 3
            # Create a new dataset from roll outs
            logger.info('Gathering rollouts...')
            new_dataset = self._gather_rollouts(self._policy, 10)
            
            ### PROBLEM 3
            # Append new dataset to original one for next training
