...
# DRL BC model (class)
class DRL_BCModel(object):

    # Constructor
    def __init__(self, _IData, _OData, _Instance, _Model, _batch_size=64, _activation=[tf.nn.tanh, tf.nn.tanh]):
        ...
        self.Model = self.createModel(self.IPlaceHolder, nNet = [self.batch_size, 32], activation=self.activation)
    ...
    def createModel(self, IPlaceHolder, nNet = [64, 32], activation = [tf.nn.tanh, tf.nn.tanh]):
        RLNModel = tf.layers.dense(inputs=IPlaceHolder, units=nNet[0], activation=activation[0], use_bias=True)
        RLNModel = tf.layers.dense(inputs=RLNModel, units=nNet[1], activation=activation[1], use_bias=True)
    ...
    def train(self, epochs=100, TrainData = None, TestData = None, nIter=None):
        ...
        opt = tf.train.AdamOptimizer().minimize(self.LossF)
